...
// RFC 7009 토큰 무효화 시도
async function tryRevokeToken(which, session, cfg) {
    const token = which === 'access' ? session.token?.access_token : session.token?.refresh_token;
    if (!token || !cfg.revoke_url)
        return;
 ...
 */
export async function oauth2Logout(request, env, providers) {
    const url = new URL(request.url);
    const revokeMode = (url.searchParams.get('revoke') || 'access');
    ...
    // 공급자 토큰 무효화
    const cfg = providers[session.provider];
