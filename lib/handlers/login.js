import { crypto, fetch } from '@cloudflare/workers-types';
...
async function fetchDiscovery(discovery) {
...
async function fetchJWKS(jwksUri) {
    const r = await fetch(jwksUri, { headers: { accept: 'application/json' }, cf: { cacheTtl: 300 } });
    if (!r.ok)
...
async function getJWKS(cfg) {
    const o = cfg.oidc;
    if (!o)
        throw new Error('no_oidc_config');
    if (o.jwks_uri)
        return fetchJWKS(o.jwks_uri);
    if (!o.discovery)
        throw new Error('no_discovery_or_jwks');
    const disc = await fetchDiscovery(o.discovery);
    return fetchJWKS(disc.jwks_uri);
}
// kid 우선, 없으면 alg + kty/crv까지 보수적으로 매칭
function pickJwkForHeader(jwks, header) {
    if (header.kid) {
    ...
    const found = jwks.keys.find(k => {
        if (isRS)
            return (k.kty === 'RSA') && (!k.alg || k.alg === 'RS256');
        if (isES)
            return (k.kty === 'EC') && (k.crv === 'P-256') && (!k.alg || k.alg === 'ES256');
        return false;
    });
    if (!found)
...
async function verifySignature(header, signature, signingInput, jwks) {
...
function assertClaims(payload, cfg, nonce) {
    const o = cfg.oidc;
    ...
    const okAud = auds.includes(cfg.client_id) ||
...
export async function oauth2Login(request, env, providers, providerKey) {
    if (request.method !== 'POST')
        return jsonWithCors({ error: 'method_not_allowed' }, 405);
    const cfg = providers[providerKey];
    if (!cfg?.oidc)
        return jsonWithCors({ error: 'unknown_or_non_oidc_provider', provider: providerKey }, 400);
    let body;
    try {
    ...
    if (!idToken)
        return jsonWithCors({ error: 'missing_idToken' }, 400);
    if (!nonce)
        return jsonWithCors({ error: 'missing_nonce' }, 400);
    try {
        // 1) JWT 파싱
