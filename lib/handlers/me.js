...
async function authnFromHeader(request, env) {
    ...
    if (!rec)
        return null;
    return { sessionId, key, recordJson: rec, record: JSON.parse(rec) };
}
/**
 * GET /me
 * - 현재 로그인 사용자 정보 반환
 ...
 */
export async function oauth2Me(request, env, providers) {
